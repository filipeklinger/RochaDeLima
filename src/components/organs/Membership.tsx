...
const Membership = ({ MembershipPlans }: { MembershipPlans: MembershipType }) => {
            ...
            <main className="grid lg:w-[90%] md:w-[96%] w-[90%] md:grid-cols-3 items-center gap-8 md:gap-4 lg:gap-8">
                {
                    MembershipPlans.cards.map((card, index) => (
                        <div onClick={() => openWhatsapp(card.contactMsg)} key={index}>
                            <Card className={`w-full flex flex-col items-center gap-4 border border-zinc-500  transition-all duration-200 cursor-pointer hover:border-red-500/50 ${index === 1 ? "lg:py-16 py-10" : "py-10"}`} >
