...
import ResBeijaflor from "../../assets/hero/res-beijaflor.jpeg"
import { Text } from "../atoms/Text"
import { List } from "../atoms/List"
import { useCallback } from "react"
import { Bank, Cardholder, Tag } from "@phosphor-icons/react"
...
const Offers = () => {
    ...
    const renderIcon = useCallback((element: number) => {
        switch (element) {
            case 0:
                return <Tag size={50} color="currentColor" weight="light" />;
            case 1:
                return <Bank size={50} color="currentColor" weight="light" />;
            case 2:
                return <Cardholder size={50} color="currentColor" weight="light" />;
            default:
                return "";
        }
    }, [])
        ...
        <section className="w-full h-auto flex items-center bg-zinc-900">
            <main className="w-full lg:h-[800px] grid md:grid-cols-2 items-center gap-10 md:gap-0 lg:gap-0 ">
                <div className="w-full md:h-[800px] h-[300px] grid">
                    <Image alt="Offer Image" objectCover="object-cover" className="w-full md:h-[800px] h-[300px]" image={ResBeijaflor} />
                </div>
